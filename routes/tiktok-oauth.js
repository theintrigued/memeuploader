  ...
  try {
    ...
    res.send(`
      <body style="font-family:sans-serif;padding:20px;">
        <h3>Success — copy this into Render's TIKTOK_REFRESH_TOKEN</h3>
        <p>Refresh token (this is the one our server actually uses):</p>
        <textarea style="width:100%;height:80px;" onclick="this.select()">${refresh_token}</textarea>
        <p>Access token (short-lived, ~${Math.round(expires_in / 3600)}h, not needed since the server auto-refreshes):</p>
